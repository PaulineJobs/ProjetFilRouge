...
#include "../../../partie2/src/JNI/JNI_JNI.h"   // Generated

JNIEXPORT void JNICALL Java_JNI_JNI_ouverture(JNIEnv *env, jobject thisObj){
    printf(" \n");
    printf(" \n");
...
void rmDescripteurs(char *repertoireParam){
	...
	//Variables
	char repertoire[strlen(repertoireParam)+1];
...
void rmListeDescripteurs(){
	...
	//remove("../../partie1/Liste_descripteurs/Liste_descripteurs_textes.txt");
	remove("../../../../partie1/Liste_descripteurs/Liste_descripteurs_audios.txt");
	remove("../../../../partie1/Liste_descripteurs/Liste_descripteurs_imageRVB.txt");
	remove("../../../../partie1/Liste_descripteurs/Liste_descripteurs_imageNB.txt");
...
void majOuverture(){
	...
	rmDescripteurs("../../../../partie1/Base_descripteurs/Base_descripteurs_audios/");
	
	//rmDescripteurs("../../partie1/Base_descripteurs/Base_descripteurs_textes");
	rmDescripteurs("../../../../partie1/Base_descripteurs/Base_descripteurs_images_RGB/");
	
	rmDescripteurs("../../../../partie1/Base_descripteurs/Base_descripteurs_images_NB/");
...
void seConnecter(){
	...
	while((choixConnexion[0]=='A')){
		...
		int connecte = verifierConnexion(motDePasse);
...
int verifierConnexion(char* motDePasse){
	
	return (strcmp(motDePasse,"1234")==0);
...
void automatisationIndexation(){
    ...
    struct dirent *dir;
    DIR *d = opendir("../../../../partie1/Corpus/AUDIO/"); 
    if (d) {
        while ((dir = readdir(d)) != NULL){
			if(strlen(dir->d_name)>4){
			//printf("je passe audio\n");
			//printf("%s\n",dir->d_name);
...
int RechercheClassique(char* souhait) {
    ...
    if (strlen(souhait) > 4) {
       for (j = 0; souhait[j + 3]; j++) {         
          if (!strcmp(&souhait[j], ".wav")){
			char chemin[strlen("../../../../partie1/Corpus/AUDIO/")+1+ strlen(souhait)];  //fichier audio
			strcpy(chemin,"../../../../partie1/Corpus/AUDIO/");
       ...
       for (j = 0; souhait[j + 3]; j++) {         
          if (!strcmp(&souhait[j], ".xml")){
			...
			char chemin[strlen("../../../../partie1/Corpus/TEXTE/")+1+ strlen(souhait)]; //fichier texte
			strcpy(chemin,"../../../../partie1/Corpus/TEXTE/");
			strcat(chemin,souhait);
			char *fichier=(chemin);
      ...
      for (j = 0; souhait[j + 3]; j++) {         
          if (!strcmp(&souhait[j], ".bmp")){
			...
			char chemin[strlen("../../../../partie1/Corpus/IMAGE/NB/")+1+ strlen(souhait)]; //fichier image NB
			strcpy(chemin,"../../../../partie1/Corpus/IMAGE/NB/");
			strcat(chemin,souhait);
			char *fichier=(chemin);
      ...
      for (j = 0; souhait[j + 3]; j++) {         
          if (!strcmp(&souhait[j], ".jpg")){
			...
			char chemin[strlen("../../../../partie1/Corpus/IMAGE/RGB/")+1+ strlen(souhait)]; //fichier RVB
...
void requeteRechercheClassique(){
	...
	int fichierConnu = RechercheClassique(souhait);
