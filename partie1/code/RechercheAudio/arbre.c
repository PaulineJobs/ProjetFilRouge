...
void stockerElement(ELEMENT e, char* stockage){
	strcat(stockage,e.nom);
	strcat(stockage," --> ");
	char ressemblance[10];
	sprintf(ressemblance, "%d\n",e.ressemblance);
	strcat(stockage,ressemblance);
}

void stockerArbre(ARBRE monArbre, char** resultat){
   ...
   //fin déclarations locales
	if (monArbre!=NULL){
		...
		//Affiche le sous arbre droit
		if(monArbre->sousArbreDroit!=NULL){
			stockerArbre(monArbre->sousArbreDroit,resultat);
		}
		
			
		//affiche la racine
		if(monArbre->document.ressemblance >= seuil){
		   stockerElement(monArbre->document,*resultat);
		}
		//affiche le sous arbre gauche
		if(monArbre->sousArbreGauche!=NULL){
			stockerArbre(monArbre->sousArbreGauche,resultat);
		
		}
		
	
	}
	//printf("stocker arbre  : %s \n", *resultat);
