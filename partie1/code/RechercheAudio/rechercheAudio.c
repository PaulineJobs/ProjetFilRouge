...
ARBRE comparaisonRepertoire(char * fichierReference){
	ARBRE ensembleFichiers=INIT_ARBRE();
	
	//On lance la recherche audio dans le repertoire contenant les descripteurs audios
    struct dirent *dir;
 
    DIR *d = opendir("../../../../partie1/Base_descripteurs/Base_descripteurs_audios/"); 
    ...
    if (d) {
        ...
        while ((dir = readdir(d)) != NULL){
			...
			if(strlen(dir->d_name)>3) {
					...
					char chemin[strlen("../../../../partie1/Base_descripteurs/Base_descripteurs_audios/")+strlen(dir->d_name)+1];
					char nomFichier[strlen(dir->d_name)+1];
					strcpy(chemin,"../../../../partie1/Base_descripteurs/Base_descripteurs_audios/");
        ...
        //afficherResultats(ensembleFichiers);
        closedir(d);
    }
...
/*
void afficherResultats(ARBRE ensembleFichiers){
	    printf("\n");
        printf(" --------------------  \n");
...
void saisieRechercheAudio(){
	...
	char *resultat;
	resultat=(char*)malloc(sizeof(char)*1000);
	resultat[0]='\0';
...
void lanceFichierAudio(char *fichierAudio) {
	...
	char *lecteurAudio="vlc --play-and-exit --global-key-quit";
	char *commande;
