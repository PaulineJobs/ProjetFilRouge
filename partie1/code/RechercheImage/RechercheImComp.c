...
ARBRE comparaisonRepertoireIm(char * fichierReference, char type){
   ...
   if(type=='1'){
      ...
      struct dirent *dir;
      DIR *d = opendir("../../../../partie1/Base_descripteurs/Base_descripteurs_images_RGB/"); 
      if (d) {
         while ((dir = readdir(d)) != NULL){
            if((strlen(dir->d_name)>3)){
               char chemin[strlen("../../../../partie1/Base_descripteurs/Base_descripteurs_images_RGB/")+strlen(dir->d_name)+1];
               char nomFichier[strlen(dir->d_name)+1];
               strcpy(chemin,"../../../../partie1/Base_descripteurs/Base_descripteurs_images_RGB/");
   ...
   {
      struct dirent *dir;
      DIR *d = opendir("../../../../partie1/Base_descripteurs/Base_descripteurs_images_NB/"); 
         if (d) {
            while ((dir = readdir(d)) != NULL){
               if((strlen(dir->d_name)>3)){
                  char chemin[strlen("../../../../partie1/Base_descripteurs/Base_descripteurs_images_NB/")+strlen(dir->d_name)+1];
                  char nomFichier[strlen(dir->d_name)+1];
                  strcpy(chemin,"../../../../partie1/Base_descripteurs/Base_descripteurs_images_NB/");
                  strcpy(nomFichier, dir->d_name);
                  strcat(chemin,nomFichier);
                  float difference=comparaison(fichierReference,chemin,type);
...
char* RechercheImgComp(char* souhait,char* typeIm){
   ...
   struct dirent *dir;
   int existe=0;
   DIR *d = opendir("../../../../partie1/Corpus/IMAGE/RGB"); 
   if (d) {
      while ((dir = readdir(d)) != NULL){
         if(!strcmp(dir->d_name,souhait)){
            souhait[strlen(souhait)-4]='\0';
            strcat(souhait,".dsc");
            char chemin[strlen("../../../../partie1/Base_descripteurs/Base_descripteurs_images_RGB/")+1+ strlen(souhait)];
            strcpy(chemin,"../../../../partie1/Base_descripteurs/Base_descripteurs_images_RGB/");
            strcat(chemin,souhait);
            existe=1;
            
            ARBRE resultatArbre = comparaisonRepertoireIm(chemin,typeIm[0]);
            
            
			result = (char*)malloc(sizeof(char)*1000);
			result[0]='\0';
			
            stockerArbre(resultatArbre, &result);
...
else
{
   struct dirent *dir;
   int existe=0;
   DIR *d = opendir("../../../../partie1/Corpus/IMAGE/NB"); 
   if (d) {
      while ((dir = readdir(d)) != NULL){
         if(!strcmp(dir->d_name,souhait)){
            souhait[strlen(souhait)-4]='\0';
            strcat(souhait,".dsc");
            char chemin[strlen("../../../../partie1/Base_descripteurs/Base_descripteurs_images_NB/")+1+ strlen(souhait)];
            strcpy(chemin,"../../../../partie1/Base_descripteurs/Base_descripteurs_images_NB/");
            strcat(chemin,souhait);
            existe=1;
...
void saisieRechercheImgComp(){
   ...
   resultat = RechercheImgComp(souhait,typeIm);
