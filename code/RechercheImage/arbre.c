...
void affiche_ELEMENTR(ELEMENTR e){
	printf("%s ",e.nom);
	printf(" --> ");
	printf("%d \n", e.ressemblance);
}
...
void affect_ELEMENTR(ELEMENTR* e1, ELEMENTR e2){
	e1->ressemblance=e2.ressemblance;
	e1->nom=e2.nom;
}

//return 1 si e1<e2
int compare_ELEMENTR(ELEMENTR e1, ELEMENTR e2){
...
void affiche_Arbre(ARBRE monArbre){
	if (monArbre!=NULL){
		...
		//affiche la racine
		affiche_ELEMENTR(monArbre->document);
...
void ajout_dans_arbre(ARBRE* monArbre, ELEMENTR doc){
	...
	ARBRE sousArbre =(ARBRE) malloc(sizeof(CELLULE));
		if (sousArbre !=NULL){
			affect_ELEMENTR(&(sousArbre->document),doc);
